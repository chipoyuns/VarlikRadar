    ...
    >
      ...
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
        ...
        >
          ...
          {!collapsed && (
            <span className="text-sm font-medium truncate">Ayarlar</span>
          )}
        </Link>

        {/* Admin - Direct Link (only for admins) */}
        {isAdmin && (
          <Link
            href="/admin"
            className="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200 relative"
      ...
      <div className="p-3 border-t border-[rgba(255,255,255,0.06)] flex-shrink-0">
        <div className="flex items-center gap-3" style={{ justifyContent: collapsed ? "center" : "flex-start" }}>
          ...
          {!collapsed && (
            <div className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-[rgba(0,212,170,0.15)] text-[#00D4AA] border border-[rgba(0,212,170,0.2)] flex-shrink-0">
              PRO
            </div>
          )}
        </div>
